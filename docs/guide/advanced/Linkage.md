...
interface rules {
  field: string; // 字段名
  control: controlType[]; // 联动内容
}

interface controlType {
  value: string; // 触发值 (当组件值跟value相等时触发)
  rule: {
    field: string, // 联动的字段key
    operator: 'show' | 'hidden' | 'required' | 'unrequired', // 联动类型
    value: string // 默认值
  };
}
```
...
::: tip
以下是一个联动示例,规则如下:
- **当性别切换为`男`,`文化程度`隐藏,`岗位`必填。**
- **当性别切换为`女`,`文化程度`显示,并且默认值为`本科`,`岗位`取消必填。**
:::
          ...
          "list": [
            {
              ...
              "key": "personname",
              "model": "personname",
              "rules": [{ "type": "string", "message": "姓名1格式不正确" }]
            }
          ]
        }
      ],
